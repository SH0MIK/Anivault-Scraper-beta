...
let ifCache = null;
async function getInfinityFreeClearance(siteUrl) {
    if (!FLARESOLVERR_URL)
        return null;
...
async function fetchUserFromSite(siteUrl, username) {
...
function buildUserEmbed(profile) {
    ...
    const ROLE_BADGE = { OWNER: '👑 Owner', owner: '👑 Owner', admin: '🛡️ Admin', mod: '🔨 Mod', user: '👤 User' };
    return {
...
async function sendDiscordFollowUp(token, body) {
    const url = `https://discord.com/api/v10/webhooks/${DISCORD_APP_ID}/${token}/messages/@original`;
    await axios_1.default.patch(url, body, { headers: { 'Content-Type': 'application/json' } });
}
...
router.post('/relay', async (req, res) => {
    ...
    try {
        const response = await axios_1.default.post(`${vercelUrl}/api/event`, req.body, {
...
//# sourceMappingURL=discord-relay.js.map