...
const anilist_1 = require("./scrapers/anilist");
const tmdb_1 = require("./scrapers/tmdb");
const kitsu_1 = require("./scrapers/kitsu");
const router = (0, express_1.Router)();
const SOURCES = ['animeheaven', 'anikoto', 'desidub'];
...
function proxiedSubtitleUrl(req, url, ref) {
    const refParam = ref ? `&ref=${encodeURIComponent(ref)}` : '';
...
function rewriteHlsPlaylist(req, body, sourceUrl, ref) {
    ...
    const proxyUri = (uri) => proxiedHlsUrl(req, new URL(uri, base).toString(), ref);
...
async function resolveAlId(anilistId, malId) {
    ...
    if (malId)
        return (0, mapper_1.malToAnilist)(parseInt(malId)); // returns null (not throw) if AniList is down
    return null;
}
...
async function resolveSiteIds(anilistId, malId) {
    ...
    if (malId)
        return (0, mapper_1.getSiteIdsByMal)(parseInt(malId));
    return null;
}
async function fetchEpisodes(source, siteIds, overrides = {}) {
    ...
    if (source === 'anikoto') {
        const slug = siteIds.siteIds?.anikoto;
        if (!slug)
            return { episodes: [], siteId: '', error: 'Not indexed on Anikoto' };
        return { episodes: await (0, anikoto_1.getAnikotoEpisodes)(slug), siteId: slug };
    }
    if (source === 'desidub') {
        const slug = siteIds.siteIds?.desidub;
        if (!slug)
            return { episodes: [], siteId: '', error: 'Not indexed on DesiDub' };
        return { episodes: await (0, desidub_1.getDesidubEpisodes)(slug), siteId: slug };
    }
...
router.get('/search', async (req, res) => {
    ...
    try {
        ...
        try {
            const malResults = await (0, mal_1.searchAnime)(q, 10);
...
router.get('/servers', async (req, res) => {
    ...
    try {
        ...
        if (source === 'anikoto')
            allServers = await (0, anikoto_1.getAnikotoServers)(episode.id);
        if (source === 'desidub')
            allServers = await (0, desidub_1.getDesidubServers)(episode.id);
...
async function watchHandler(req, res) {
    ...
    try {
        ...
        const filtered = type === 'all'
            ? allServers
            : allServers.filter((s) => s.type === type);
        if (!filtered.length)
        ...
        if (preferredServer && strict) {
            candidates = filtered.filter((s) => s.name.toLowerCase().includes(preferredServer.toLowerCase()));
            if (!candidates.length) {
                return res.status(404).json({ error: `No server matching "${preferredServer}" found`, availableServers: filtered.map((s) => s.name) });
            }
        }
        ...
        for (const server of candidates) {
            ...
            if (source === 'anikoto')
                raw = await (0, anikoto_1.getAnikotoEmbedUrl)(server.sourceId);
            if (source === 'desidub')
                raw = await (0, desidub_1.getDesidubStream)(server.sourceId);
            if (raw) {
                embedResult = raw;
        ...
        if (!embedResult) {
            ...
            return res.status(502).json({ error: msg, triedServers: candidates.map((s) => s.name) });
        }
        if (source === 'animeheaven') {
            return res.json({
                anilistId: siteIds.anilistId,
        ...
        if (source === 'anikoto') {
            return res.json({
                ...
                subtitles: (embedResult.subtitles ?? []).map((s) => ({
                    ...s,
...
router.get('/mal/anime/:id', async (req, res) => {
    ...
    try {
        const details = await (0, mal_1.getAnimeDetails)(malId);
        if (!details)
            return res.status(404).json({ error: 'Anime not found on MAL' });
...
router.get('/mal/anime/:id/episodes', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    ...
    try {
        const result = await (0, mal_1.getEpisodes)(malId, page);
...
router.get('/mal/anime/:id/episodes/:epNum', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    const epNum = parseInt(req.params.epNum, 10);
    if (isNaN(malId) || isNaN(epNum))
        return res.status(400).json({ error: 'id and epNum must be numbers' });
    try {
        const result = await (0, mal_1.getEpisode)(malId, epNum);
        if (!result)
            return res.status(404).json({ error: `Episode ${epNum} not found` });
...
router.get('/mal/search', async (req, res) => {
    ...
    try {
        const result = await (0, mal_1.searchAnime)(q, isNaN(limit) ? 8 : limit);
        return res.json({ data: result });
    }
    catch (e) {
...
router.get('/mal/search/debug', async (req, res) => {
    const q = req.query.q;
    if (!q)
        return res.status(400).json({ error: 'Missing ?q=' });
    try {
        const result = await (0, mal_1.debugSearchHtml)(q);
        return res.json(result);
    }
    catch (e) {
...
router.get('/mal/anime/:id/external', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getExternalLinks)(malId);
        return res.json({ data: result });
    }
    catch (e) {
...
router.get('/mal/anime/:id/characters', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getCharacters)(malId);
        return res.json({ data: result });
    }
    catch (e) {
...
router.get('/mal/character/:id', async (req, res) => {
    const charId = parseInt(req.params.id, 10);
    if (isNaN(charId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getCharacterDetails)(charId);
        if (!result)
            return res.status(404).json({ error: 'Character not found on MAL' });
        return res.json(result);
    }
    catch (e) {
        return res.status(502).json({ error: 'MAL character scrape failed', detail: e?.message || String(e) });
...
router.get('/mal/anime/:id/pictures', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getAnimePictures)(malId);
        return res.json({ data: result });
    }
    catch (e) {
...
router.get('/mal/character/:id/pictures', async (req, res) => {
    const charId = parseInt(req.params.id, 10);
    if (isNaN(charId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getCharacterPictures)(charId);
        return res.json({ data: result });
    }
    catch (e) {
        return res.status(502).json({ error: 'MAL picture scrape failed', detail: e?.message || String(e) });
...
router.get('/mal/anime/:id/themes', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getAnimeThemes)(malId);
        return res.json(result);
    }
    catch (e) {
...
router.get('/mal/anime/:id/videos', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getAnimeVideos)(malId);
        return res.json(result);
    }
    catch (e) {
...
router.get('/mal/anime/:id/streaming', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getStreamingPlatforms)(malId);
        return res.json({ data: result });
    }
    catch (e) {
...
router.get('/mal/anime/:id/recommendations', async (req, res) => {
    const malId = parseInt(req.params.id, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'id must be a number' });
    try {
        const result = await (0, mal_1.getRecommendations)(malId);
        return res.json({ data: result });
    }
    catch (e) {
...
router.get('/anilist/season', async (req, res) => {
    try {
        const result = await (0, anilist_1.getSeasonNow)();
...
router.get('/anilist/top-banners', async (req, res) => {
    try {
        const limit = parseInt(req.query.limit, 10) || 200;
        const result = await (0, anilist_1.getTopBanners)(limit);
...
router.get('/anilist/id', async (req, res) => {
    ...
    try {
        const anilistId = await (0, mapper_1.malToAnilist)(malId);
        if (!anilistId)
            return res.status(404).json({ error: 'Not found on AniList', malId });
...
router.get('/anilist/episodes', async (req, res) => {
    const malId = parseInt(req.query.malId, 10);
    if (isNaN(malId))
        return res.status(400).json({ error: 'malId required' });
    try {
        const episodes = await (0, anilist_1.getStreamingEpisodes)(malId);
...
router.get('/anilist/anime', async (req, res) => {
    ...
    try {
        const { result, log } = await (0, anilist_1.getAnimeImages)(malId, isList);
        if (!result)
            return res.status(404).json({ error: 'No AniList images found', log });
...
function computeTmdbTitleCandidates(rawTitles, log) {
    let seasonHint = null;
    const baseTitles = [];
    for (const t of rawTitles) {
        const { base, season } = (0, tmdb_1.extractSeasonHint)(t);
        if (season !== null && seasonHint === null)
            seasonHint = season;
...
async function resolveTmdbTitles(req, log) {
    ...
    if (rawTitle) {
        ...
        if (!details)
            return null;
        rawTitles = [...new Set([details.titleEnglish, details.title, details.titleJapanese].filter((t) => !!t))];
    }
...
router.get('/tmdb/episode-thumb', async (req, res) => {
    const epNum = parseInt(req.query.ep, 10);
    if (isNaN(epNum))
        return res.status(400).json({ error: 'Missing/invalid ?ep=' });
    if (!req.query.title && isNaN(parseInt(req.query.malId, 10))) {
    ...
    try {
        ...
        if (!resolved)
            return res.status(404).json({ error: 'MAL ID not found', log });
        for (const title of resolved.titles) {
            const { result, log: srcLog } = await (0, tmdb_1.getEpisodeThumbnail)(title, epNum, resolved.seasonHint, isList);
            log.push(...srcLog);
            if (result)
                return res.json({ data: result, log });
        }
...
router.get('/tmdb/anime', async (req, res) => {
    ...
    try {
        ...
        if (!resolved)
            return res.status(404).json({ error: 'MAL ID not found', log });
        for (const title of resolved.titles) {
            const { result, log: srcLog } = await (0, tmdb_1.getAnimeImages)(title, resolved.seasonHint, isList);
            log.push(...srcLog);
            if (result)
                return res.json({ data: result, log });
        }
...
async function resolveKitsuAnimeId(req, log) {
    const rawTitle = req.query.title;
    const malId = parseInt(req.query.malId, 10);
...
router.get('/kitsu/episode-thumb', async (req, res) => {
    const epNum = parseInt(req.query.ep, 10);
    if (isNaN(epNum))
        return res.status(400).json({ error: 'Missing/invalid ?ep=' });
    if (!req.query.title && isNaN(parseInt(req.query.malId, 10))) {
    ...
    try {
        ...
        if (kitsuAnimeId === 'mal-not-found')
            return res.status(404).json({ error: 'MAL ID not found', log });
        if (!kitsuAnimeId)
            return res.status(404).json({ error: 'No Kitsu anime match found', log });
        const { result, log: srcLog } = await (0, kitsu_1.getEpisodeThumbnail)(kitsuAnimeId, epNum, isList);
        log.push(...srcLog);
        if (!result)
            return res.status(404).json({ error: 'No Kitsu episode thumbnail found', log });
        return res.json({ data: result, log });
    }
    catch (e) {
...
router.get('/kitsu/anime', async (req, res) => {
    if (!req.query.title && isNaN(parseInt(req.query.malId, 10))) {
        return res.status(400).json({ error: 'Provide ?title= or ?malId=' });
    }
    const isList = req.query.list === '1';
    ...
    try {
        ...
        if (!kitsuAnimeId)
            return res.status(404).json({ error: 'No Kitsu anime match found', log });
        const { result, log: srcLog } = await (0, kitsu_1.getAnimeImages)(kitsuAnimeId, isList);
        log.push(...srcLog);
        if (!result)
            return res.status(404).json({ error: 'No Kitsu images found', log });
        return res.json({ data: result, log });
    }
    catch (e) {
...
function matchAnilistEpisode(episodes, epNum) {
...
async function resolveEpisode(malId, epNum, details, isList, anilistEpisodes, knownMalEpisode) {
    const log = [];
    const primaryTitle = details ? (details.titleEnglish || details.title) : null;
    const malEpisode = knownMalEpisode !== undefined
        ? knownMalEpisode
        : await (0, mal_1.getEpisode)(malId, epNum).catch((e) => {
    ...
    if (details) {
        const rawTitles = [...new Set([details.titleEnglish, details.title, details.titleJapanese].filter((t) => !!t))];
        const { titles, seasonHint } = computeTmdbTitleCandidates(rawTitles, log);
        for (const t of titles) {
            const { result, log: srcLog } = await (0, tmdb_1.getEpisodeData)(t, epNum, seasonHint, isList, malEpisode?.aired ?? null);
            log.push(...srcLog);
            if (result) {
                if (result.title) {
                    title = result.title;
        ...
        if (!title)
            log.push('Episode info: not found on TMDB, trying MAL');
        if (!thumbnail)
            log.push('Thumbnail: not found on TMDB, trying Kitsu');
    }
    else {
    ...
    if (!thumbnail || !title) {
        try {
            const kitsuAnimeId = await (0, kitsu_1.getKitsuAnimeId)(malId, primaryTitle, log);
            if (kitsuAnimeId) {
                const { result } = await (0, kitsu_1.getEpisodeData)(kitsuAnimeId, epNum, isList);
                if (result) {
    ...
    if (!title || !thumbnail) {
        try {
            const episodes = anilistEpisodes ?? (await (0, anilist_1.getStreamingEpisodes)(malId));
...
async function resolveAnimeArt(malId, details, isList) {
    const log = [];
    const primaryTitle = details ? (details.titleEnglish || details.title) : null;
    let poster = null;
    ...
    if (details) {
        ...
        const { titles, seasonHint } = computeTmdbTitleCandidates(rawTitles, log);
        for (const t of titles) {
            const { result, log: srcLog } = await (0, tmdb_1.getAnimeImages)(t, seasonHint, isList);
            log.push(...srcLog);
            if (result) {
                if (result.poster) {
        ...
        if (!poster)
            log.push('Poster: not found on TMDB, trying Kitsu');
        if (!cover)
            log.push('Cover: not found on TMDB, trying Kitsu');
        if (!logo)
            log.push('Logo: not found on TMDB (no fallback -- TMDB is the only logo source)');
    }
    else {
    ...
    if (!poster || !cover) {
        try {
            const kitsuAnimeId = await (0, kitsu_1.getKitsuAnimeId)(malId, primaryTitle, log);
            if (kitsuAnimeId) {
                const { result } = await (0, kitsu_1.getAnimeImages)(kitsuAnimeId, isList);
                if (result) {
                    if (!poster && result.poster) {
                        poster = result.poster;
                        posterSource = 'kitsu';
                        log.push('Poster: found via Kitsu');
                    }
                    if (!cover && result.cover) {
                        cover = result.cover;
                        coverSource = 'kitsu';
                        log.push('Cover: found via Kitsu');
                    }
                }
                if (!poster)
                    log.push('Poster: not found on Kitsu, trying AniList');
                if (!cover)
                    log.push('Cover: not found on Kitsu, trying AniList');
            }
            else {
    ...
    if (!poster || !cover) {
        try {
            const { result } = await (0, anilist_1.getAnimeImages)(malId, isList);
            if (result) {
                if (!poster && result.poster) {
                    poster = result.poster;
                    posterSource = 'anilist';
                    log.push('Poster: found via AniList');
                }
                if (!cover && result.cover) {
                    cover = result.cover;
                    coverSource = 'anilist';
                    log.push('Cover: found via AniList');
                }
            }
            if (!poster)
                log.push('Poster: not found on any source');
            if (!cover)
                log.push('Cover: not found on any source');
        }
        catch (e) {
            log.push(`AniList lookup failed (${e?.message})`);
        }
...
router.get('/anime', async (req, res) => {
    ...
    try {
        const details = await (0, mal_1.getAnimeDetails)(malId);
        if (!details)
            return res.status(404).json({ error: 'MAL ID not found' });
    ...
    try {
        ...
        if (hasEp) {
            ...
            if (!info.thumbnail && !info.title)
                return res.status(404).json({ error: 'No episode data found from any source', log: info.log });
            return res.json({
                data: {
                    malId,
                    episode: epNum,
        ...
        // Whole show
        const concurrency = Math.min(Math.max(parseInt(req.query.concurrency, 10) || 5, 1), 10);
        const malEpisodes = await (0, mal_1.getAllEpisodes)(malId).catch((e) => {
            throw new Error(`MAL episode list fetch failed: ${e?.message || e}`);
        });
        if (!malEpisodes.length)
            return res.status(404).json({ error: 'No episodes found on MAL for this malId' });
        const anilistEpisodes = await (0, anilist_1.getStreamingEpisodes)(malId).catch(() => []);
        ...
        if (malScrapeLooksIncomplete) {
            ...
            if (details) {
                ...
                for (const t of titles) {
                    const found = await (0, tmdb_1.getShowEpisodeCount)(t).catch(() => null);
                    if (found) {
                        tmdbCount = found.count;
                        break;
                    }
                }
            }
...
router.get('/health', (_req, res) => {
    ...
    const stats = (0, cache_1.cacheStats)();
    const process_ = {
        ok: true,
